from tkinter import *
import mysql.connector

import tkinter.messagebox as msg

def T_conect():
    return mysql.connector.connect(
        host="localhost",
        user="root",
        password="",
        database="python_Demo")
def insert_data():
    if e_fname.get()=="" or e_lname.get()=="" or e_email.get()=="" or e_number.get()=="":
        msg.showinfo("Insert Status","All Fields Are Mandatory")
    else:
        conn=T_conect()
        cursor=conn.cursor()
        query="insert into student(fname,lname,email,mobile) values(%s,%s,%s,%s)"
        args=(e_fname.get(),e_lname.get(),e_email.get(),e_number.get())
        cursor.execute(query,args)
        conn.commit()
        conn.close()
        e_fname.delete(0,'end')
        e_lname.delete(0,'end')
        e_email.delete(0,'end')
        msg.showinfo("Insert Status","Data Inserted Successfully")
def serch_data():
    e_fname.delete(0,'end')
    e_lname.delete(0,'end')
    e_email.delete(0,'end')
    e_number.delete(0,'end')
    if e_id=="":
        msg.showinfo("Search Status","ID is Mandatory")
    else:
        conn=T_conect()
        cursor=conn.cursor()
        query="select * from student where id =%s"
        args=(e_id.get(),)
        cursor.execute(query,args)
        row=cursor.fetchall()
        if row:
            for i in row:
                e_fname.insert(0,i[1])
                e_lname.insert(0,i[2])
                e_email.insert(0,i[3])
                e_number.insert(0,i[4])
        else:
            msg.showinfo("Search Status","Data Not Found")
        conn.close()
def update_data():
    if e_id.get()=="" or e_fname.get()=="" or e_lname.get()=="" or e_email.get()=="" or e_number.get()=="":
        msg.showinfo("Update Status","All Fields Are Mandatory")
    else:
        conn=T_conect()
        cursor=conn.cursor()
        cursor = conn.cursor()
        query = "update student set fname=%s,lname=%s,email=%s,mobile=%s where id=%s"
        args = (e_fname.get(), e_lname.get(), e_email.get(), e_number.get(), e_id.get())
        cursor.execute(query, args)
        conn.commit()
        conn.close()
        e_id.delete(0, 'end')
        e_fname.delete(0, 'end')
        e_lname.delete(0, 'end')
        e_email.delete(0, 'end')
        e_number.delete(0, 'end')
        msg.showinfo("Update Status", "Data Updated Successfully")


def delete_data():
    if e_id.get()=="":
        msg.showinfo("Delete Status","Id is Mandatory")
    else:
        conn=T_conect()
        cursor=conn.cursor()
        qurey="delete from student where id=%s"
        args=(e_id.get(),)
        cursor.execute(qurey,args)
        conn.commit()
        conn.close()
        e_id.delete(0,'end')
        e_fname.delete(0,'end')
        e_lname.delete(0,'end')
        e_email.delete(0,'end')
        e_number.delete(0,'end')
        msg.showinfo("Delete Status","Data Deleted Sucessfully")

root=Tk()
root.geometry("400x400")
root.title("My tkinter App")
root.configure(bg="light gray")
root.resizable(width=False,height=False)


l_id=Label(root,text="ID")
l_id.place(x=50,y=50)

l_fname=Label(root,text="FIRST NAME")
l_fname.place(x=50,y=100)

l_lname=Label(root,text="LAST NAME")
l_lname.place(x=50,y=150)

l_email=Label(root,text="EMAIL")
l_email.place(x=50,y=200)

l_number=Label(root,text="MOBILE NUMER")
l_number.place(x=50,y=250)

e_id=Entry(root)
e_id.place(x=170,y=50)

e_fname=Entry(root)
e_fname.place(x=170,y=100)

e_lname=Entry(root)
e_lname.place(x=170,y=150)

e_email=Entry(root)
e_email.place(x=170,y=200)

e_number=Entry(root)
e_number.place(x=170,y=250)


insert=Button(root,text="INSERT",bg="black",fg="white",font="Arial 10",command=insert_data)
insert.place(x=50,y=300)

search=Button(root,text="SEARCH",bg="black",fg="white",font="Arial 10",command=serch_data)
search.place(x=125,y=300)

update=Button(root,text="UPDATE",bg="black",fg="white",font="Arial 10",command=update_data)
update.place(x=211,y=300)

Del=Button(root,text="DELETE",bg="black",fg="white",font="Arial 10",command=delete_data)
Del.place(x=294,y=300)




root.mainloop()
